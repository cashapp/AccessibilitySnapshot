...
/// omit off-screen elements is made later, at delivery time (see `AccessibilityDelivery`). Only
/// the parser has the UIKit context (scroll offsets, inset-adjusted bounds) needed to compute it,
/// so it is recorded here rather than derived downstream.
