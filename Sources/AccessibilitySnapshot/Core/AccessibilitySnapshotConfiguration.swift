...
public struct AccessibilitySnapshotConfiguration {
    ...
    /// Whether to render a hierarchical legend that groups elements by their accessibility containers. Defaults to `false`.
    /// This flag only affects the legend — the snapshot image and element overlays are unchanged.
    ...
    ///   - showContainers: Whether to render a hierarchical legend grouping elements by container. Defaults to `false`.
    public init(
