...
internal extension AccessibilitySnapshotView {
    final class LegendView: UIView {
        ...
        init(marker: AccessibilityMarker, color: UIColor, showUserInputLabels: Bool) {
            ...
            // If our description and hint are both empty, and we don't have custom actions or content, but we do have custom rotors, we'll use the description label
            // to show the "More Content Available" text, since this makes our layout simpler when we align to the marker.
            let showCustomRotorsInDescription = (marker.description.isEmpty &&
                                                 !showActionsAvailableInDescription &&
                                                 marker.customContent.isEmpty &&
                                                 !showCustomContentInDescription &&
                                                 !marker.customRotors.isEmpty )

            self.customRotorsView = marker.customRotors.isEmpty ? nil : .init(
                    customRotorsText: showCustomRotorsInDescription
                        ? nil
                    : Strings.rotorsAvailableText(rotorNames: marker.customRotors.compactMap({$0.name}), for: marker.accessibilityLanguage),
                    rotors: marker.customRotors,
