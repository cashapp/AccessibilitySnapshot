        ...
        func assign(_ nodes: [AccessibilityHierarchy]) -> [AssignedNode] {
            nodes.map { node in
                ...
                case let .element(element, traversalIndex):
                    guard let index = traversalIndexToPosition[traversalIndex] else {
                        assertionFailure("Element traversalIndex \(traversalIndex) missing from index map")
                        return .element(element, colorIndex: 0)
                    }
